...
import CadastrarAgencia from "../../pages/agencias/CadastrarAgencia";
...
vi.mock("../../components/Form/Form_Agencia", () => ({
  default: () => <div data-testid="form-agencia" />,
}));
...
describe("CadastrarAgencia", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("bloqueia envio quando ID do usuario logado nao existe", async () => {
    vi.mocked(getId).mockReturnValue(undefined);

    const { container } = renderWithProviders(<CadastrarAgencia />);
    const form = container.querySelector("form.containerForm");

    expect(activePage).toHaveBeenCalledWith("agencias");
  ...
  it("envia para createUser quando nao ha imagem e usuario valido", async () => {
